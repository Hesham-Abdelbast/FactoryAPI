...
namespace FactoryAPI.Controllers.Employees
{
    ...
    public class EmployeeManagementController(IEmployeeManagementService service, ILogger<EmployeeManagementController> logger) : ControllerBase
    {
        ...
        [HttpPost("{employeeId:guid}/{year:int}/{month:int}")]
        public async Task<ActionResult<TResponse<Guid>>> GeneratePayroll(Guid employeeId, int year, int month)
        {
            try
            {
                var id = await service.GeneratePayrollAsync(employeeId, year, month);
                return Ok(new TResponse<Guid> { Success = true, Data = id, ReturnMsg = "📌 تم إنشاء كشف المرتب" });
            }
            catch (Exception ex)
            {
                logger.LogError(ex, $"{nameof(GeneratePayroll)}");
                return Ok(new TResponse<Guid> { Success = false, ReturnMsg = ex.Message });
            }
        }

        [HttpGet("{employeeId:guid}/{year:int}/{month:int}")]
        public async Task<ActionResult<TResponse<EmployeeMonthlyPayrollDto>>> GetPayroll(Guid employeeId, int year, int month)
        {
            try
            {
                var data = await service.GetPayrollAsync(employeeId, year, month);
                return Ok(new TResponse<EmployeeMonthlyPayrollDto> { Success = true, Data = data });
            }
            catch (Exception ex)
            {
                logger.LogError(ex, $"{nameof(GetPayroll)}");
                return Ok(new TResponse<EmployeeMonthlyPayrollDto> { Success = false, ReturnMsg = ex.Message });
            }
        }
        ...
        public async Task<ActionResult<TResponse<EmployeeFinancialReportDto>>> FinancialReport(Guid employeeId, DateTime? from = null, DateTime? to = null)
        {
            try
            {
                var report = await service.GetEmployeeFinancialSummaryAsync(employeeId, from, to);
                return Ok(new TResponse<EmployeeFinancialReportDto> { Success = true, Data = report });
            }
            catch (Exception ex)
            {
                logger.LogError(ex, $"{nameof(FinancialReport)}");
                return Ok(new TResponse<EmployeeFinancialReportDto> { Success = false, ReturnMsg = ex.Message });
            }
        }
